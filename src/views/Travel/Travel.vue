<template>
  ...
  <!-- 按钮 -->
  <div class="btn">
    <van-button type="primary" block @click="search">查询</van-button>
  </div>
  <!-- 结果内容 -->
  <div class="content">
    <div><b>出发城市:</b></div>
...
export default {
  ...
  setup() {
    ...
    const onFinish1 = ({ value, selectedOptions }) => {
      code.to = value
      show1.value = false
      fieldValue1.value = selectedOptions.map((option) => option.text).join('/')
    }
    const search = () => {
      if (fieldValue1.value && fieldValue.value) {
        console.log(code)
        api.getCitys(code).then((res) => {
          console.log(res)
          fromInfo.value = res.data.result.from_info.low_in_desc
          toInfo.value = res.data.result.to_info.low_in_desc
        })
      }
    }
